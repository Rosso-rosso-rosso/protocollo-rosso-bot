...
from bot.node_registry import InMemoryNodeRegistry, NodeRegistryStore
...
class FabricNode:
    def __init__(self, identity: NodeIdentity, registry: NodeRegistryStore, *, fabric_id: str = "r3-interna-rossorosso", scopes: set[str] | None = None, capabilities: set[str] | None = None, router: MemoryFabricRouter | None = None):
    ...
    def receive(self, envelope: dict[str, Any]) -> tuple[str, Any]:
        ...
        sender = self.registry.get(envelope["node_id"])
        if not sender: return "REJECT_UNKNOWN_NODE", None
        key = next((item for item in sender["keys"] if item["key_id"] == envelope["key_id"]), None)
